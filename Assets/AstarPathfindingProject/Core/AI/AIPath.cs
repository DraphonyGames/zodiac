...
public class AIPath : MonoBehaviour {
	...
	public float pickNextWaypointDist = 2.5f;
	
	/** Target point is Interpolated on the current segment in the path so that it has a distance of #forwardLook from the AI.
	  * See the detailed description of AIPath for an illustrative image */
	public float forwardLook = 0f;
	 ...
	 */
	public float endReachedDistance = 2.5F;
	...
	protected Vector3 CalculateVelocity (Vector3 currentPosition) {
		...
		if (/*currentWaypointIndex >= vPath.Count-3 && */ targetDist <= endReachedDistance) {
    ...
    protected virtual void RotateTowards(Vector3 dir, bool slerp)
    {
        Quaternion rot = slerp ? Quaternion.Slerp(tr.rotation, Quaternion.LookRotation(dir), turningSpeed * Time.fixedDeltaTime) : Quaternion.LookRotation(dir);
