...
#region Fields
		...
		public void LoadFromCache () {
			if (data_cachedStartup != null && data_cachedStartup.Length > 0) {
				...
				Debug.LogError ("[AstarData.cs] Can't load from cache since the cache is empty");
			}
		}
			...
			try {
				if (bytes != null) {
					...
					if (sr.OpenDeserialize(bytes)) {
						...
						Debug.Log ("[AstarData.cs] Invalid data file (cannot read zip). Trying to load with old deserializer (pre 3.1)...");
						AstarSerializer serializer = new AstarSerializer (active);
                ...
                Debug.LogWarning("[AstarData.cs] Caught exception while deserializing data.\n" + e);
				data_backup = bytes;
			}
		}
			...
			try {
				if (bytes != null) {
					...
					if (sr.OpenDeserialize(bytes)) {
                        ...
                        Debug.Log("[AstarData.cs] Invalid data file (cannot read zip).");
					}
				} else {
					throw new System.ArgumentNullException ("Bytes should not be null when passed to DeserializeGraphs");
				}
			...
			for (int i=0;i<graphs.Length;i++) {
				for (int j=i+1;j<graphs.Length;j++) {
					if (graphs[i] != null && graphs[j] != null && graphs[i].guid == graphs[j].guid) {
                        Debug.LogWarning("[AstarData.cs] Guid Conflict when importing graphs additively. Imported graph will get a new Guid.\nThis message is (relatively) harmless.");
...
#region OldSerializer
		...
		public void DeserializeGraphs_oldInternal (AstarSerializer serializer, byte[] bytes) {
			...
			if (bytes == null || bytes.Length == 0) {
                Debug.Log("[AstarData.cs] No previous data, assigning default");
				graphs = new NavGraph[0];
				return;
			}

            Debug.Log("[AstarData.cs] Deserializing...");
            ...
            Debug.Log("[AstarData.cs] Deserialization complete - Process took " + ((endTime - startTime).Ticks * 0.0001F).ToString("0.00") + " ms");
		}
		...
		public void DeserializeGraphsPart (AstarSerializer serializer) {
			...
			if (serializer.error != AstarSerializer.SerializerError.Nothing) {
                ...
                Debug.Log("[AstarData.cs] Error encountered : " + serializer.error + "\nWriting data to AstarData.data_backup");
				graphs = new NavGraph[0];
				return;
			}
			...
			try {
				...
				if (count1 != count2) {
                    Debug.LogError("[AstarData.cs] Data is corrupt (" + count1 + " != " + count2 + ")");
					graphs = new NavGraph[0];
					return;
				}
				...
				for (int i=0;i<_graphs.Length;i++) {
					...
					if (!serializer.MoveToAnchor ("Graph"+i)) {
                        Debug.LogError("[AstarData.cs] Couldn't find graph " + i + " in the data");
                        Debug.Log("[AstarData.cs] Logging... " + serializer.anchors.Count);
						foreach (KeyValuePair<string,int> value in serializer.anchors) {
                            Debug.Log("[AstarData.cs] KeyValuePair " + value.Key);
						}
						_graphs[i] = null;
						continue;
					}
					...
					if (graph == null) {
                        Debug.LogError("[AstarData.cs] One of the graphs saved was of an unknown type, the graph was of type '" + graphType + "'");
						data_backup = data;
						graphs = new NavGraph[0];
						return;
					}
				...
				data_backup = (serializer.readerStream.BaseStream as System.IO.MemoryStream).ToArray ();
                Debug.LogWarning("[AstarData.cs] Deserializing Error Encountered - Writing data to AstarData.data_backup:\n" + e.ToString());
				graphs = new NavGraph[0];
				return;
			}
		}
...
#region GraphCreation
		...
		public NavGraph CreateGraph (string type) {
            Debug.Log("[AstarData.cs] Creating Graph of type '" + type + "'");
            ...
            Debug.LogError("[AstarData.cs] Graph type (" + type + ") wasn't found");
			return null;
		}
		...
		public NavGraph AddGraph (string type) {
			...
			if (graph == null) {
                Debug.LogError("[AstarData.cs] No NavGraph of type '" + type + "' could be found");
				return null;
			}
		...
		public NavGraph AddGraph (System.Type type) {
			...
			if (graph == null) {
                Debug.LogError("[AstarData.cs] No NavGraph of type '" + type + "' could be found, " + graphTypes.Length + " graph types are avaliable");
				return null;
			}
...
#region GraphUtility
		...
		public Node GetNode (int graphIndex, int nodeIndex, NavGraph[] graphs) {
			...
			if (graphIndex < 0 || graphIndex >= graphs.Length) {
                Debug.LogError("[AstarData.cs] Graph index is out of range" + graphIndex + " [0-" + (graphs.Length - 1) + "]");
				return null;
			}
			...
			if (nodeIndex < 0 || nodeIndex >= graph.nodes.Length) {
                Debug.LogError("[AstarData.cs] Node index is out of range : " + nodeIndex + " [0-" + (graph.nodes.Length - 1) + "]" + " (graph " + graphIndex + ")");
				return null;
			}
		...
		public int GetGraphIndex (NavGraph graph) {
            ...
            Debug.LogError("[AstarData.cs] Graph doesn't exist");
			return -1;
		}
